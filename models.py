...
# def cosine_similarity_loss(latent1, latent2):
#     # Normalize latent vectors to unit length
#     latent1_norm = tf.nn.l2_normalize(latent1, axis=-1)
#     latent2_norm = tf.nn.l2_normalize(latent2, axis=-1)
#
#     # Cosine similarity
#     cosine_similarity = tf.reduce_sum(latent1_norm * latent2_norm, axis=-1)
#
#     # Cosine distance (minimize 1 - cosine similarity)
...
class JeanZADAutoencoder(tf.keras.Model):
    def __init__(self, batch_size_train, batch_size_valid, latent_dim=32, ocsvm_coeff=0.1, nu_ocsvm_coeff=0.03,
        ...
        super(JeanZADAutoencoder, self).__init__()
        self.latent_dim = latent_dim
